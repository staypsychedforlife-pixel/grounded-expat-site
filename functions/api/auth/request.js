...
import { sign, json, hasActiveSubscription } from "./_shared.js";
...
export async function onRequestPost({ request, env }) {
  ...
  if (isMember) {
    ...
    await sendMagicEmail(email, link, env.RESEND_API_KEY);
  }
  // Same response whether or not they're a member.
  return json({ ok: true });
}

async function sendMagicEmail(to, link, resendKey) {
  if (!resendKey) return;
  const html = `<div style="font-family:Helvetica,Arial,sans-serif;color:#2A2622;font-size:16px;line-height:1.6;max-width:460px">
    <p>Hi there,</p>
    <p>Here's your one-tap link to sign in to <b>The Grounded Expat</b>. It works once and expires in 15 minutes.</p>
    <p style="margin:26px 0"><a href="${link}" style="background:#C2674A;color:#fff;text-decoration:none;padding:14px 28px;border-radius:100px;font-weight:600;display:inline-block">Sign in</a></p>
